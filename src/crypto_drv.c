...
/* helpers */
void hmac_md5(char *key, int klen, char *dbuf, int dlen, 
                     char *hmacbuf);
void hmac_sha1(char *key, int klen, char *dbuf, int dlen, 
                      char *hmacbuf);
...
///STATIC NOW-- removed dyn init code

ErlDrvRWLock** lock_vec = NULL; /* Static locks used by openssl */
...
*/
unsigned char* return_binary(char **rbuf, int rlen, int len)
{
    if (len <= rlen) {
...
unsigned char* return_binary_shrink(char **rbuf, int rlen, unsigned char* data, int len)
{
...
void locking(int mode, ErlDrvRWLock* lock)
{
    switch(mode) {
...
void dyn_lock_function(int mode, struct CRYPTO_dynlock_value* ptr,const char *file, int line)
{
    locking(mode, (ErlDrvRWLock*)ptr);
}
void dyn_destroy_function(struct CRYPTO_dynlock_value *ptr, const char *file, int line)
{
...
void hmac_md5(char *key, int klen, char *dbuf, int dlen, char *hmacbuf)
{
    MD5_CTX ctx;
